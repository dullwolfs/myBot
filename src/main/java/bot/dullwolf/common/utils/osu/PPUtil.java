        ...
        if(!"".equals(param)){
            ...
            if(split.length == 3){
                try {
                    ...
                    if(acc > 100 || acc < 80){
                        flag = false;
                        CmdUtil.sendMessage("acc范围值在80-100之间", id, type);
                    }
                    if(allMod.contains(split[1].toUpperCase().substring(0,2))){
                        mod = split[1];
                    }else{
                        CmdUtil.sendMessage("大佬！你输入的mod格式不对，系统已默认设置为None", id, type);
                    }
                } catch (NumberFormatException e) {
                    flag = false;
                    CmdUtil.sendMessage("请检查歌谱id或者acc是否为数字类型的参数！", id, type);
                ...
                try {
                    ...
                    flag = false;
                    CmdUtil.sendMessage("请检查歌谱id是否为数字类型的参数！", id, type);
                }
            }else{
                CmdUtil.sendMessage("没有第四个参数了吧？！", id, type);
            }
            if(flag){
                StatsPP.stats(id, type, bid, mod, acc);
            }
        }else{
            CmdUtil.sendMessage("必须要输入歌谱ID的参数！", id, type);
        }
    }
